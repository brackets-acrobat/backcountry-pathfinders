...
--    utilisateurs ──< vols                (une sortie = un envoi groupé de posers)
--    utilisateurs ──< lieux (créateur)
--    vols  ──< releves        (chaque relevé appartient à un vol, vol_id NOT NULL)
--    lieux ──< releves        (un relevé = une contribution : profil + sol + perf)
...
-- ------------------------------------------------------------
--  Vols : une sortie pilotée (regroupe les posers d'un même vol).
--  Envoyée en bloc par l'appli desktop ; consultée sur le site.
-- ------------------------------------------------------------
...
--  Chaque relevé appartient à un vol (vol_id NOT NULL) : supprimer un vol
--  retire ses relevés (CASCADE), puis l'app nettoie les lieux devenus vides.
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS releves (
    id               INT UNSIGNED NOT NULL AUTO_INCREMENT,
    id_lieu          INT UNSIGNED NOT NULL,
    vol_id           INT UNSIGNED NOT NULL,                 -- vol auquel appartient ce poser
